...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a VPS initiative\n"
            "  vote-alias         - Vote on a VPS initiative\n"
            "  vote               - Vote on a VPS initiative/budget\n"
    ...
    if (strCommand == "prepare") {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start vps_address monthly_payment_vps'");
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start vps_address monthly_payment_vps fee_tx'");
