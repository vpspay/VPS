...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::VPS, nValue);
...
{
    std::string genesisblockhash = "0000088c0254d8db142adaa4c372ec10523d1f1a7ca2316bd9121524c63b3fb9";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (vpspay.conf).");
            QMessageBox::warning(this, "VPSPAY Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
